...
class ETHMugsDataset(Dataset):
    def __init__(self, root_dir, mode="train"):
        ...
        # 2. Was wird gemacht, wenn man von "train" oder "val" data redet.
        ...
        # 3. Was wird gemacht, wenn man von "test" data redet.
        if self.mode == "test":
            self.rgb_dir = os.path.join(self.root_dir, "rgb")
            self.image_paths = [f for f in os.listdir(self.rgb_dir) if f.endswith(".jpg")]
            self.transform = transforms.Compose([
        ...
        print("[INFO] Dataset mode:", mode)  # Gibt aus, welcher Mode genutzt wird (train/test/val).
        print("[INFO] Number of images in the ETHMugDataset:", len(self.image_paths))  # Gibt die Anzahl der Bilder im Datensatz aus.

    def __len__(self):
        # Ermöglicht len(dataset); gibt die Anzahl der Bilder zurück.
    ...
    def __getitem__(self, idx: int):
        ...
        image = self.transform(image)  # Wendet die Transformationen auf das Bild an.

        if self.mode in ["train", "val"]:  # Im Training und Validierung werden echte Masken geladen.
            ...
            mask = transforms.ToTensor()(mask)  # Wandelt die Maske in einen Tensor um.
        else:
            # Gibt Dummy-Maske zurück, falls nicht train oder val.
