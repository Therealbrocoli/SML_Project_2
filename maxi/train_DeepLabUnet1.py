...
from utils import *
from DeepLabUnet1 import DeepLabUnet  ### Neu: importiert das erweiterte Modell
import math  ### hinzugefügt, um isnan abzufangen
...
def train(ckpt_dir: str, train_data_root: str, val_data_root: str, config: dict):
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=config['hyperparameters']['train_batch_size'],  ### geändert: train_batch_size statt batch_size
        shuffle=True,
        num_workers=2,
        pin_memory=True
    )
    ...
    # 10. Early-Stopping-Variablen initialisieren  ###
    best_val_iou = -1.0  ###
    epochs_no_improve = 0  ###
    es_patience = config['hyperparameters']['es_patience']  ### Early-Stopping basiert auf eigener Geduld
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for image, gt_mask in val_loader:
                ...
                # ### Batch-Dimension aufsplitten und individuelle IoU berechnen ###
                for b in range(preds.shape[0]):  ###
                    pred_2d = (preds[b, 0].cpu().numpy() > 0.5).astype(np.uint8)  ###
                    gt_2d   = gt_mask[b, 0].cpu().numpy().astype(np.uint8)       ###
                    iou_val = compute_iou(pred_2d, gt_2d)                         ###
                    if math.isnan(iou_val):  ### abgefangen, falls union==0
                        iou_val = 0.0                                             ###
                    val_iou += iou_val                                           ###
        ...
        if epochs_no_improve >= es_patience:  ###
            print(f"{RED}[INFO]: Early stopping triggered at epoch {epoch} (no improvement for {es_patience} epochs).{RESET}")  ###
            break  ###

        # 11.6 Checkpoint speichern
        if (epoch % 3 == 0):  ### geänderte Häufigkeit (nur jedes 3. Epoch)
            t = time.perf_counter()
    ...
    # 12. Plotting von Loss und Validation-IoU
    t = time.perf_counter()
    ...
    # 13. Test-Inferenz & CSV-Erzeugung
    t = time.perf_counter()
    model.eval()
    image_ids = []
    pred_masks = []
    ...
    to_tensor = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize([0.427, 0.419, 0.377], [0.234, 0.225, 0.236])    # Nutzt zuvor berechnetes mean/std
    ])  ###
    ...
    for fname in sorted(os.listdir(td)):
        ...
        img_path = p_jpg if os.path.exists(img_path := p_jpg) else p_png  ### gepatcht: Sicherstellen, dass img_path gesetzt wird
    ...
    # 14. Speichern der Predictions als CSV
    t = time.perf_counter()
